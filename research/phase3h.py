...
from research.phase3g import PHASE3G_DATASET_VERSION


PHASE3H_DATASET_VERSION = "driftbench-real-adjudication-holdout-phase3h-v1"
PHASE3H_SPLIT_VERSION = "phase3h-group-safe-gold-holdout-v1"
PROVENANCE_SCHEMA_VERSION = "phase3h-provenance-v1"

PHASE3H_REPOSITORIES = [
    {
        "repo": "ClearURLs/Addon",
        "extension_id": "github:clearurls/addon",
        "extension_name": "ClearURLs",
        "functional_category": "privacy_security",
        "category_source": "repository_project_description",
        "license": "LGPL-3.0",
        "asset_contains": "ClearURLs",
        "asset_suffix": ".zip",
        "max_releases": 6,
        "split": "test",
        "external_holdout": True,
        "source_family": "public_github_release_assets",
    },
    {
        "repo": "FirefoxBar/HeaderEditor",
        "extension_id": "github:firefoxbar/headereditor",
        "extension_name": "HeaderEditor",
        "functional_category": "developer_tools",
        "category_source": "repository_project_description",
        "license": "MIT",
        "asset_contains": "HeaderEditor-",
        "asset_suffix": "-v3.xpi",
        "max_releases": 6,
        "split": "test",
        "external_holdout": True,
    ...
    },
    {
        ...
        "extension_id": "github:ruffle-rs/ruffle",
        "extension_name": "Ruffle Web Extension",
        "functional_category": "media",
        "category_source": "repository_project_description",
        "license": "MIT OR Apache-2.0",
        "asset_contains": "web-extension",
        "asset_suffix": ".zip",
        "max_releases": 8,
    ...
    },
    {
        ...
        "extension_id": "github:bitwarden/clients-browser",
        "extension_name": "Bitwarden Browser",
        "functional_category": "password_security_helper",
        "category_source": "repository_project_description",
        "license": "GPL-3.0",
        "asset_prefix": "dist-firefox-",
        "asset_suffix": ".zip",
        "max_releases": 2,
        "split": "test",
        "source_family": "public_github_release_assets",
...
PHASE3H_SECURITY_EXCLUDED_EXTENSION_IDS: Dict[str, str] = {}


def run_phase3h_maturation(
    *,
    phase3g_feature_dir: str | Path = "artifacts/driftbench/phase3g_features",
    phase3g_curation_dir: str | Path = "artifacts/driftbench/phase3g",
    phase3g_experiment_dir: str | Path = "artifacts/experiments/phase3g",
    incoming_dir: str | Path = "datasets/incoming/phase3h",
    validated_dir: str | Path = "datasets/validated/phase3h_packages",
    manifest_path: str | Path = "datasets/manifests/phase3h_import_manifest.json",
    curation_dir: str | Path = "artifacts/driftbench/phase3h",
    feature_dir: str | Path = "artifacts/driftbench/phase3h_features",
    experiment_dir: str | Path = "artifacts/experiments/phase3h",
    seed: int = 1337,
    max_download_bytes: int = 20 * 1024 * 1024,
) -> Dict[str, Any]:
    started = time.perf_counter()
    phase3g_feature_dir = Path(phase3g_feature_dir)
    phase3g_curation_dir = Path(phase3g_curation_dir)
    phase3g_experiment_dir = Path(phase3g_experiment_dir)
    ...
    phase3g_reference = freeze_phase3g_reference(phase3g_curation_dir, phase3g_feature_dir, phase3g_experiment_dir)
    if manifest_path.exists():
        cached_manifest = read_json(manifest_path)
        new_releases = cached_manifest.get("new_release_assets", {})
    else:
        new_releases = acquire_phase3h_release_assets(
    ...
    excluded_releases = {
        extension_id: {
            "reason": PHASE3H_SECURITY_EXCLUDED_EXTENSION_IDS[extension_id],
            "release_count": len(releases),
            "releases": releases,
        }
        for extension_id, releases in new_releases.items()
        if extension_id in PHASE3H_SECURITY_EXCLUDED_EXTENSION_IDS
    }
    new_releases = {
        extension_id: releases
        for extension_id, releases in new_releases.items()
        if extension_id not in PHASE3H_SECURITY_EXCLUDED_EXTENSION_IDS
    }

    records = build_phase3h_records(phase3g_curation_dir, new_releases)
    ...
    analyzer_missingness = build_analyzer_missingness(records, phase3g_feature_dir)
    ...
    gold_set = build_gold_set_manifest(records)
    holdout = build_external_holdout_manifest(records)
    temporal_coverage = build_temporal_coverage(records)
    dataset_statistics = build_dataset_statistics(records, provenance_manifest)
    dataset_quality_report = build_dataset_quality_report(
        dataset_manifest,
        readiness,
        provenance_manifest,
        label_quality_report,
        analyzer_missingness,
        gold_set,
        holdout,
    )

    write_json(curation_dir / "phase3g_reference.json", phase3g_reference)
    ...
    write_json(curation_dir / "analyzer_health.json", analyzer_missingness)
    write_json(curation_dir / "dataset_statistics.json", dataset_statistics)
    write_json(curation_dir / "dataset_quality_report.json", dataset_quality_report)
    write_json(curation_dir / "gold_set_manifest.json", gold_set)
    write_json(curation_dir / "external_holdout_manifest.json", holdout)
    write_json(curation_dir / "temporal_coverage.json", temporal_coverage)
    write_json(curation_dir / "adjudication_report.json", build_adjudication_report(records))
    ...
    write_phase3h_import_manifest(manifest_path, records, new_releases, excluded_releases)

    if validation.errors:
        raise ValueError(f"Phase 3H validation failed: {validation.errors}")
    if not duplicate_report["passed"]:
        raise ValueError("Phase 3H duplicate audit failed")
    if not leakage_report["passed"]:
        raise ValueError("Phase 3H leakage audit failed")

    non_holdout_records = [record for record in records if not is_external_holdout(record)]
    if phase3h_feature_cache_valid(feature_dir, expected_records=len(non_holdout_records)):
        feature_status = read_json(feature_dir / "extraction_manifest.json")
    else:
        new_records = [
            record for record in records
            if "Phase 3H corpus maturation record" in record.notes and not is_external_holdout(record)
        ]
        extraction = DriftBenchFeatureExtractor(base_dir=".").extract(new_records)
        if not extraction.is_valid:
            raise ValueError(f"Phase 3H feature extraction failed: {extraction.validation_errors or extraction.failed_records}")
        write_phase3h_feature_artifacts(
            phase3g_feature_dir,
            extraction,
            feature_dir,
    ...
    if not feature_leakage_audit["passed"]:
        raise ValueError("Phase 3H feature leakage audit failed")
    write_json(experiment_dir / "feature_leakage_audit.json", feature_leakage_audit)

    comparison_rows = build_phase3g_vs_phase3h_dataset_comparison(phase3g_curation_dir, dataset_manifest, readiness, gold_set, holdout)
    write_csv(experiment_dir / "phase3g_vs_phase3h_dataset.csv", comparison_rows)

    dataset_snapshot = build_dataset_snapshot(records, phase3g_reference, curation_dir, dataset_manifest, provenance_manifest, gold_set, holdout)
    experiment_manifest = {
        "phase": "Phase 3H",
        ...
        "split_version": PHASE3H_SPLIT_VERSION,
    ...
    write_json(experiment_dir / "phase3g_reference.json", phase3g_reference)
    ...
    return {
        ...
        "phase3g_reference": phase3g_reference,
    }


def acquire_phase3h_release_assets(*, incoming_dir: Path, validated_dir: Path, max_download_bytes: int) -> Dict[str, List[Dict[str, Any]]]:
    acquired: Dict[str, List[Dict[str, Any]]] = {}
    for spec in PHASE3H_REPOSITORIES:
        releases = public_github_releases(spec["repo"], per_page=12)
        selected = []
        for release in releases:
            if release.get("draft") or (release.get("prerelease") and not spec.get("allow_prerelease")):
                continue
        ...
        for item in selected:
            ...
            records.append({
                ...
                "split": spec["split"],
                "external_holdout": bool(spec.get("external_holdout")),
            })
        acquired[spec["extension_id"]] = records
    return acquired


def build_phase3h_records(phase3g_curation_dir: Path, new_releases: Dict[str, List[Dict[str, Any]]]) -> List[DatasetPairRecord]:
    phase3g_manifest = read_json(phase3g_curation_dir / "dataset_manifest.json")
    phase3g_records = [record_from_dict(row) for row in phase3g_manifest["records"]]
    phase3h_records = []
    for releases in new_releases.values():
        for old, new in zip(releases, releases[1:]):
            phase3h_records.append(record_from_release_pair(old, new))
    return phase3g_records + phase3h_records


def record_from_dict(row: Dict[str, Any]) -> DatasetPairRecord:
    provenance = row["provenance"]
    notes = list(row.get("notes", []))
    notes.append("PHASE3G_FROZEN_BASELINE")
    return DatasetPairRecord(
        ...
        provenance=ProvenanceRecord(
            ...
            notes=list(provenance.get("notes", [])) + ["Preserved unchanged from frozen Phase 3G dataset history."],
        ),
...
def record_from_release_pair(old: Dict[str, Any], new: Dict[str, Any]) -> DatasetPairRecord:
    ...
    notes = [
        "Phase 3H corpus maturation record",
    ...
    if new.get("external_holdout") or new.get("split") == "external_holdout":
        notes.append("EXTERNAL_REPLICATION_HOLDOUT")
    return DatasetPairRecord(
        pair_id=pair_id,
        ...
        provenance=ProvenanceRecord(
            ...
            license=new["license"],
            collector="codex-phase3h-public-github-release-assets",
        ...
        split="test" if new.get("split") == "external_holdout" else new["split"],
        notes=notes,
    )
...
def build_provenance_manifest(records: Sequence[DatasetPairRecord], new_releases: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
    ...
    for record in records:
        payload_records.append({
            ...
            "is_phase3h_record": "Phase 3H corpus maturation record" in record.notes,
            "is_PHASE3G_FROZEN_BASELINE": "PHASE3G_FROZEN_BASELINE" in record.notes,
        })
    return {
...
def reviewer_metadata_for(record: DatasetPairRecord) -> Dict[str, Any]:
    return {
        ...
        "independent_second_reviewer_id": None,
            ...
            "blind_review_status": "initial_packet_excludes_driftwatch_score_and_model_output",
...
def build_dataset_manifest(
    ...
    PHASE3H_count = sum(1 for record in records if "Phase 3H corpus maturation record" in record.notes)
    return {
        "dataset_version": PHASE3H_DATASET_VERSION,
        "previous_dataset_version": PHASE3G_DATASET_VERSION,
        ...
        "new_phase3h_record_count": PHASE3H_count,
        ...
        "source_family_distribution": {"public_github_release_assets": len(records)},
...
def build_readiness(
    ...
    if dataset_manifest["real_record_count"] < 75:
        warnings.append("planning target of 75+ real records was not reached")
    if dataset_manifest["unique_extension_count"] < 20:
        warnings.append("planning target of 20+ unique extensions was not reached")
    ...
    return {
        "dataset_version": PHASE3H_DATASET_VERSION,
        "previous_dataset_version": PHASE3G_DATASET_VERSION,
        "phase3h_complete": not block_reasons,
        "recommended_next_phase": (
            "CONTINUE INDEPENDENT LABEL REVIEW / ADJUDICATION"
            if not block_reasons and dataset_manifest["real_record_count"] >= 75 and dataset_manifest["unique_extension_count"] >= 20
            else "CONTINUE REAL-DATA EXPANSION"
        ),
...
def build_analyzer_missingness(records: Sequence[DatasetPairRecord], phase3g_feature_dir: Path) -> Dict[str, Any]:
    counts = Counter()
    rows = load_csv(phase3g_feature_dir / "full_driftwatch.csv")
    for row in rows:
...
def build_review_packet(record: DatasetPairRecord) -> Dict[str, Any]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="phase3h_review_") as workspace:
...
def build_second_review_queue(records: Sequence[DatasetPairRecord]) -> Dict[str, Any]:
    ...
    for record in records:
        ...
        if record.label == "uncertain":
            score += 90
        if "Phase 3H corpus maturation record" in record.notes:
            score += 20
    ...
    return {
        "schema_version": "phase3h-second-review-queue-v1",
...
def reviewer_schema() -> Dict[str, Any]:
    return {
        "schema_version": "phase3h-reviewer-schema-v1",
        "fields": [
            "reviewer_id",
...
def build_gold_set_manifest(records: Sequence[DatasetPairRecord]) -> Dict[str, Any]:
    eligible = [
        record for record in records
        if record.label_quality_tier in {"CONTROLLED_GROUND_TRUTH", "EXTERNAL_CONFIRMED", "MULTI_REVIEWER_ADJUDICATED"}
        and record.label not in {"uncertain", "excluded"}
    ]
    return {
        "dataset_version": PHASE3H_DATASET_VERSION,
        "gold_set_policy": "Only externally confirmed, controlled-ground-truth, or genuine multi-reviewer-adjudicated records qualify.",
        "gold_record_count": len(eligible),
        "label_distribution": dict(Counter(record.label for record in eligible)),
        "records": [
            {
                "record_id": record.pair_id,
                "extension_id": record.extension_id,
        ...
        "exclusion_reason": "No real Phase 3H records currently have external confirmation or genuine multi-reviewer adjudication." if not eligible else None,
    }


def is_external_holdout(record: DatasetPairRecord) -> bool:
    return "EXTERNAL_REPLICATION_HOLDOUT" in record.notes


def build_external_holdout_manifest(records: Sequence[DatasetPairRecord]) -> Dict[str, Any]:
    holdout = [record for record in records if is_external_holdout(record)]
    return {
        "dataset_version": PHASE3H_DATASET_VERSION,
        "holdout_name": "EXTERNAL_REPLICATION_HOLDOUT",
        "split_policy": PHASE3H_SPLIT_VERSION,
        "creation_timestamp": datetime.now(timezone.utc).isoformat(),
        "record_count": len(holdout),
        "unique_extension_count": len({record.extension_id for record in holdout}),
        "label_distribution": dict(Counter(record.label for record in holdout)),
        "label_visibility_policy": "Holdout labels are retained in curation metadata but holdout rows are excluded from non-holdout feature artifacts and must not be used for tuning.",
        ...
        "records": [
            {
                ...
                "new_sha256": record.provenance.new_sha256,
                "split": record.split,
                "holdout_status": "EXTERNAL_REPLICATION_HOLDOUT",
            }
...
def build_temporal_coverage(records: Sequence[DatasetPairRecord]) -> Dict[str, Any]:
    dated = [record for record in records if record.old_timestamp and record.new_timestamp]
    years = Counter((record.new_timestamp or "")[:4] for record in dated)
    return {
        "timestamp_complete_count": len(dated),
        "record_count": len(records),
        "new_transition_year_distribution": dict(sorted(years.items())),
        "temporal_holdout_status": "external_holdout_locked; prospective temporal validation deferred to Phase 3I",
        "timestamp_source_policy": "GitHub release published_at or preserved trusted release metadata; filesystem timestamps are not used.",
    }


def build_dataset_statistics(records: Sequence[DatasetPairRecord], provenance_manifest: Dict[str, Any]) -> Dict[str, Any]:
    by_extension = Counter(record.extension_id for record in records)
    return {
        "record_count": len(records),
        "real_record_count": len(records),
        "controlled_record_count": 0,
        "unique_extension_count": len(by_extension),
        "pairs_per_extension": dict(sorted(by_extension.items())),
        "label_distribution": dict(Counter(record.label for record in records)),
        "label_quality_distribution": dict(Counter(record.label_quality_tier for record in records)),
        "license_distribution": dict(Counter(record.license for record in records)),
        "category_distribution": dict(Counter(record.functional_category or "unknown" for record in records)),
        "source_family_distribution": {"public_github_release_assets": len(records)},
        "split_distribution": dict(Counter(record.split for record in records)),
        "provenance_completeness": provenance_manifest["provenance_complete_count"],
        "timestamp_completeness": provenance_manifest["timestamp_complete_count"],
    }
...
def build_dataset_quality_report(
    ...
    total = dataset_manifest["accepted"]
    provisional = dataset_manifest["label_quality_tier_distribution"].get("SINGLE_REVIEWER_PROVISIONAL", 0)
    return {
        "dataset_version": PHASE3H_DATASET_VERSION,
        "dimensions": {
            "provenance_completeness": f"{provenance_manifest['provenance_complete_count']}/{total}",
            "timestamp_completeness": f"{provenance_manifest['timestamp_complete_count']}/{total}",
            "labeling_strength": label_quality_report["label_quality_distribution"],
            "single_reviewer_provisional_percentage": round(100 * provisional / total, 6) if total else 0,
            "review_coverage": {
                "double_reviewed": label_quality_report["double_reviewed_count"],
                "inter_rater_available": False,
            },
            "extension_diversity": dataset_manifest["unique_extension_count"],
            "source_diversity": dataset_manifest["source_family_distribution"],
            "split_integrity": {
                "duplicate_audit_passed": readiness["duplicate_audit_passed"],
                "leakage_audit_passed": readiness["leakage_audit_passed"],
            },
            "duplicate_cleanliness": readiness["duplicate_audit_passed"],
            "analyzer_coverage": analyzer_missingness,
            "gold_set_size": gold_set["gold_record_count"],
            "external_holdout_size": holdout["record_count"],
        },
        "bias_analysis": {
            "open_source_bias": "Corpus remains open-source-repository biased.",
            "github_source_bias": "All accepted Phase 3H records currently come from public GitHub release assets.",
            "benign_class_dominance": "Benign transitions remain dominant.",
            "risky_ground_truth_scarcity": "Risky/review-worthy labels remain scarce.",
            "malicious_ground_truth_scarcity": "No independently confirmed malicious-transition packages are present.",
            "reviewer_bias": "Most labels remain single-reviewer provisional.",
        },
    }


def build_adjudication_report(records: Sequence[DatasetPairRecord]) -> Dict[str, Any]:
    return {
        "dataset_version": PHASE3H_DATASET_VERSION,
        "adjudicated_count": 0,
        "disagreement_count": 0,
        "history_preserved": True,
        "status": "NO_GENUINE_MULTI_REVIEWER_ADJUDICATION_AVAILABLE",
        "records": [],
    }


def write_phase3h_feature_artifacts(
    phase3g_feature_dir: Path,
    new_extraction,
    output_dir: Path,
    *,
    ...
    paths: Dict[str, str] = {}
    schema_payload = read_json(phase3g_feature_dir / "feature_schema.json")
    ...
    for baseline in BASELINES:
        existing_rows = load_csv(phase3g_feature_dir / f"{baseline}.csv")
    ...
    summary = {
        ...
        "phase3g_parent_rows_reused": len(load_csv(phase3g_feature_dir / "full_driftwatch.csv")),
        "phase3h_new_rows_extracted": len(new_extraction.feature_rows),
    }
    ...
    manifest = {
        ...
        "source_dataset_identifier": "DriftBench Phase 3H",
        "source_dataset_version": PHASE3H_DATASET_VERSION,
        "previous_dataset_version": PHASE3G_DATASET_VERSION,
        "phase3g_parent_feature_reuse": True,
        "phase3h_new_records_extracted_only": True,
        "model_training_performed": False,
        "production_ml_integration": False,
...
def phase3h_feature_cache_valid(feature_dir: Path, *, expected_records: int) -> bool:
    ...
    try:
        ...
        return False
    return (
        manifest.get("source_dataset_version") == PHASE3H_DATASET_VERSION
        and manifest.get("record_count") == expected_records
        and all((feature_dir / f"{baseline}.csv").exists() for baseline in BASELINES)
    )


def build_phase3g_vs_phase3h_dataset_comparison(
    phase3g_curation_dir: Path,
    phase3h_manifest: Dict[str, Any],
    phase3h_readiness: Dict[str, Any],
    gold_set: Dict[str, Any],
    holdout: Dict[str, Any],
) -> List[Dict[str, Any]]:
    phase3g = read_json(phase3g_curation_dir / "dataset_manifest.json")
    phase3g_label_quality = phase3g["label_quality_tier_distribution"]
    phase3h_label_quality = phase3h_manifest["label_quality_tier_distribution"]
    return [
        {
            "metric": "total_real_records",
            "phase3g": phase3g["real_record_count"],
            "phase3h": phase3h_manifest["real_record_count"],
        },
        {
            "metric": "unique_extensions",
            "phase3g": phase3g["unique_extension_count"],
            "phase3h": phase3h_manifest["unique_extension_count"],
        },
        {
            "metric": "risky_or_malicious_records",
            "phase3g": phase3g["label_distribution"].get("risky_transition", 0) + phase3g["label_distribution"].get("malicious_transition", 0),
            "phase3h": phase3h_manifest["risky_review_worthy_count"],
        },
        {
            "metric": "uncertain_records",
            "phase3g": phase3g["label_distribution"].get("uncertain", 0),
            "phase3h": phase3h_manifest["uncertain_count"],
        },
        {
            "metric": "double_reviewed_count",
            "phase3g": phase3g_label_quality.get("MULTI_REVIEWER_ADJUDICATED", 0),
            "phase3h": phase3h_label_quality.get("MULTI_REVIEWER_ADJUDICATED", 0),
        },
        {
            "metric": "provisional_label_percentage",
            "phase3g": round(100 * phase3g_label_quality.get("SINGLE_REVIEWER_PROVISIONAL", 0) / phase3g["accepted"], 6),
            "phase3h": round(100 * phase3h_label_quality.get("SINGLE_REVIEWER_PROVISIONAL", 0) / phase3h_manifest["accepted"], 6),
        },
        {
            "metric": "provenance_completeness",
            "phase3g": "58/58",
            "phase3h": phase3h_readiness["provenance_completeness"],
        },
        {
            "metric": "timestamp_completeness",
            "phase3g": "58/58",
            "phase3h": phase3h_readiness["timestamp_completeness"],
        },
        {
            "metric": "gold_set_size",
            "phase3g": 0,
            "phase3h": gold_set["gold_record_count"],
        },
        {
            "metric": "external_holdout_size",
            "phase3g": 0,
            "phase3h": holdout["record_count"],
        },
        {
            "metric": "train_validation_test_counts",
            "phase3g": json.dumps(phase3g["split_counts"], sort_keys=True),
            "phase3h": json.dumps(phase3h_manifest["split_counts"], sort_keys=True),
        },
    ]
...
def build_dataset_snapshot(
    records: Sequence[DatasetPairRecord],
    phase3g_reference: Dict[str, Any],
    curation_dir: Path,
    dataset_manifest: Dict[str, Any],
    provenance_manifest: Dict[str, Any],
    gold_set: Dict[str, Any],
    ...
    return {
        "dataset_version": PHASE3H_DATASET_VERSION,
        "previous_dataset_version": PHASE3G_DATASET_VERSION,
        "phase3g_frozen_baseline": phase3g_reference,
        "schema_version": "phase3d-intake-v1",
        "ontology_version": "phase3d-labels-v1",
        "feature_schema_version": "1.0",
        "split_version": PHASE3H_SPLIT_VERSION,
        ...
        "real_record_count": len(records),
        "controlled_record_count": 0,
        "new_phase3h_record_count": dataset_manifest["new_phase3h_record_count"],
        "unique_extension_count": len({record.extension_id for record in records}),
        "label_distribution": dict(Counter(record.label for record in records)),
        "label_quality_distribution": dict(Counter(record.label_quality_tier for record in records)),
        ...
        "external_holdout_extension_ids": sorted({record.extension_id for record in records if is_external_holdout(record)}),
...
def freeze_phase3g_reference(curation_dir: Path, feature_dir: Path, experiment_dir: Path) -> Dict[str, Any]:
    snapshot = read_json(experiment_dir / "dataset_snapshot.json")
    return {
        "status": "FROZEN_PHASE3G_BASELINE",
        ...
        "preservation_policy": "Phase 3H reads Phase 3G as immutable research history and writes only phase3h outputs.",
    }


def write_phase3h_import_manifest(
    path: Path,
    records: Sequence[DatasetPairRecord],
